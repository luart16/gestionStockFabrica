...
import { Request, Response } from "express";      
 ...
 export const modificarStockProductoPorId = async (req: Request, res: Response) => {
     try{
         const {id} = req.params;
         const {cantidad, enStock} = req.body;
         const stock = await StockProducto.findById(id);
         if (!stock) {
        ...
        console.log(error);
         res.status(500).json({ error: 'Error al modificar el stock de producto.'})
     }
 }
